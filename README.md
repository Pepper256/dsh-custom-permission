...
| 权限（面板） | YAML 字段 | 含义 | 常用写法 |
|---|---|---|---|
| 自动允许 | `allowRules` | 命中即**跳过审批直接放行**（含命令级） | 工具模式（见下）+ 可选条件 |
| 自动拒绝 | `denyRules` | 命中即**直接失败**，模型看到拒绝文案；优先级高于允许 | 工具模式（见下）+ 可选条件 + 可选拒绝原因 |
| 自动放行工具 | `allowApprovals` | 审批请求（含沙箱升级）**自动通过**的工具名单 | 每行一个**精确工具名**（`fetch`、`write`；**不支持通配符/正则**） |
| 额外可写路径 | `extraWritableRoots` | 沙箱外允许读写（`workspace-write` 下）的路径 | 每行一个绝对路径（相对路径按后端 `cwd` 解析） |

### 规则字段精讲（自动允许 / 自动拒绝）

每条规则 = **工具模式** + 可选的**条件**（针对工具参数）：

- **工具模式**（在 UI 中称为"工具模式"输入框；YAML 的 `tool`）：匹配"哪个工具"，支持三种写法：
  - **精确名**：`bash`、`write`、`web_search` —— 只有同名工具命中（大小写敏感）；
  - **glob**：含通配符时按 glob 匹配，支持 `*`（不跨 `/`）、`**`、`?`、`[set]`、`{a,b}`，如 `fs_*`、`tool_*`、`{read,write}_*`；
  - **正则**：以 `regex:` 开头，后面是 JavaScript 正则，如 `regex:^web_`（匹配 `web_search`、`web_fetch` 等）。
  - 留空的一行在保存时会被丢弃（用于删除多余行）。
- **条件**（YAML 的 `when`）：进一步限定"什么样的参数才命中"。每个条件三要素：
  - **字段**（YAML 的键）：要检查的工具参数名，用点路径访问嵌套参数。常见：shell 工具的命令文本填 `command`；文件系统工具的目标路径填 `path`；其余工具直接填其参数名（如 `url`）。**参数的整个字符串**才参与匹配，参数缺失或不是字符串则该条件不命中。
  - **条件类型**：把字段值同下面哪种规则比较——

| 类型 | 含义 | 示例（字段 `command`） | 命中示例 |
|---|---|---|---|
| contains | 值作为**子串**出现即可 | `DANGER_MARKER` | 任何含该子串的命令 |
| regex | JavaScript **正则**（未锚定，任意位置匹配；需要全串匹配请用 `^…$`） | `rm -rf /` | 命令文本任意位置含 `rm -rf /` |
| prefix | 值必须是字段值的**开头** | `git status` | 以 `git status` 开头的命令 |
| glob | 字段**整个值**按 glob 匹配（支持 `*` `**` `?` `[set]` `{a,b}`） | `git *` | 以 `git ` 开头的整串命令（`*` 不跨 `/`） |

  - **值**：比较对象文本。空白值的一行在保存时会被丢弃。
  - 组合语义：一个规则里多条条件之间是 **AND**（全部命中才算命中）；同一字段也可有多条不同类型条件（彼此 AND）。保存时正则会校验，**非法正则直接报错、不会保存**。
- **拒绝原因**（仅自动拒绝规则有；YAML 的 `reason`）：可选文案，命中时作为工具错误展示给模型（覆盖默认的 `blocked by dsh-custom-permission rule #N (tool "...")`）。
- 求值顺序：按列表顺序，**第一条命中的规则生效**；同一预设内 deny 永远优先于 allow。

示例：想"拒绝 pwsh 里执行带 `rm -rf /` 的命令并给出原因"→ 自动拒绝加一条规则：工具模式 `pwsh`，条件字段 `command`、类型 `contains`、值 `rm -rf /`，拒绝原因写 `禁止递归删除`。

> 切换、持久化与种子语义见下一节；预设名规则：非空、不含空白、≤64 字符、不与现有预设重名（创建与编辑时都会校验）。
