...
- **Web GUI 按钮**：聊天输入框右侧的 ⚙ 按钮（浏览器半，随 profile 挂载即出现、无需重建 web 应用）打开预设面板——列出全部预设、点击切换（进程级、所有会话生效）；出错时面板提示"检查并修复 cordis.patch.yml"。面板内的"快捷添加"按钮目前是占位（不打开配置文件，快捷添加即将支持）。
